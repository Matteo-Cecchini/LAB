...
for i, j in zip(os.listdir(directory), colori):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("figure", filename))
    plt.show()
    '''
    coeff = rfft(data[1])
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("figure", filename + "_spe"))
    plt.show()
    '''
    # sintesi
    '''
    ...
    plt.tight_layout()
    plt.savefig(os.path.join("figure", filename + "_syn"))
    plt.show()
    '''
    ...
    plt.xlim(0,.1)
    plt.legend()
    plt.savefig(os.path.join("figure", filename + "_fil"))
    plt.show()